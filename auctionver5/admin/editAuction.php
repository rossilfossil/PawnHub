...
<body>	
			...
			<div class="row">
				...
				<div class="col m6 l6 s12">	
	         		...
	         		<input type="text" name="modalName" id="modalName" value="<?php echo $get_row['auction_name']?>" required>
	         	</div>
				<div class="col m6 l6 s12">		
	         		<label for="modalDesc">Auction Description</label>
	         		<input type="text" name="modalDesc" id="modalDesc" value="<?php echo $get_row['auction_description']?>" required>
				</div>
			</div>		
<!-- 			<div class="row">
				<div class="col s12">
         			<label for="modalPrice">Price</label>
         			<input type="number" name="modalPrice" step="0.01" id="modalPrice" value="<?php //echo $get_row['auction_']?>" required>
				</div>	
			</div> -->
			<div class="row">
				<div class="col s12 m6 l6">
         			<label for="modalSDate">Start Date</label>
					<input type="date" id="modalDate" name="modalSDate" class="validate" min="<?php echo $today?>" oninput="setMaxDate(this.value);document.getElementById('enddate').min=this.value;" value="<?php echo $get_row['start_date']?>" required>
				</div>
				<div class="col s12 m6 l6">
         			<label for="modalSTime">Start Time</label>
					<input type="time" id="modalTime" name="modalSTime" class="validate" value="<?php echo $get_row['start_time']?>" required>
				</div>
			</div>
			<div class="row">
				<div class="col s12 m6 l6">	
         			<label for="enddate">End Date</label>
					<input type="date" id="enddate" name="enddate" class="validate" min="<?php echo $today?>" onchange="document.getElementById('modalDate').max=this.value" value="<?php echo $get_row['end_date']?>" required>         	
				</div>
				<div class="col s12 m6 l6">
					<label for="modalETime">End Time</label>
					<input type="time" id="modalETime" name="modalETime" class="validate" value="<?php echo $get_row['end_time']?>" required>
				</div>
			</div>			
         	<button class="right btn black white-text" type="submit" name="edit">Edit</button>
         	</form>
</body>
</html>