<?php
	...
	if(!isset($_POST['pangcheck']) OR $reportType == 2){
		$title = "Bids for this month, ".$monthnames[$month-1]." ".$year;
		$start = $year."-".$month."-01";
		$end = $year."-".$month."-31";
		$get2 = mysql_query("SELECT * FROM tbl_Bids 
			WHERE bid_date >= '$start' AND bid_date <= '$end'
			");
			// if(!mysql_num_rows($get2) ==0){
				...
				for ($i=0; $i < $monthend[$month-1]; $i++) { 
					...
					$m = $i+1;
					$start = $year."-".$month."-".$m;
					$get3 = mysql_query("SELECT * FROM tbl_Bids 
						WHERE bid_date = '$start'
					");
...
?>    
	...
	<div class="row">
	<h3><center>Bids Report</center></h3>
	<div class="container">
		...
		<div class="col l4 s12 m12">
			<div class="row">
				<form action="" method="POST">
					<div class="col s12">
    						<input class="black" type="radio" id="reportType3" name="reportType" value="2"/>
    						<label class="black-text" for="reportType3">Daily</label><BR><BR>
    						<input class="black" type="radio" id="reportType2" name="reportType" value="0"/>
    						<label class="black-text" for="reportType2">Monthly</label><BR><BR>
    						<input class="black" type="radio" id="reportType1" name="reportType" value="1"/>
    ...
    $('#graph').highcharts({
        ...
        tooltip: {
            ...
            pointFormat: '<tr><td style="color:{series.color};padding:0"></td>' +
                '<td style="padding:0"><b> {point.y:.1f} Bids</b></td></tr>',
            footerFormat: '</table>',
            shared: true,
            useHTML: true
        },
