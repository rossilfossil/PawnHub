						...
						?>
									<tr class="black-text">
										...
										<form action="" method="POST">
											...
											<input type="hidden" name="auction" value="<?php echo $get_row['auction_ID']?>">
...
<?php
	if(isset($_POST['send'])){
		...
		if (!$mode==0){	
			...
			$sql = "SELECT tbl_Items.item_ID AS 'itemId' , tbl_items.itemId AS 'secondaryId' FROM tbl_auctions, tbl_auctionitems, tbl_items 
			WHERE tbl_auctionitems.item_ID = tbl_items.item_ID 
					AND tbl_auctions.auction_ID = tbl_auctionitems.auction_ID 
					AND tbl_auctions.auction_ID = $auctionId";
			$res = mysql_query($sql) or die("Error in Query: ".mysql_error());
			while($row = mysql_fetch_assoc($res))
			{
				...
				$sql = "UPDATE tbl_Item SET itemStatus = 10 WHERE itemId = $secondaryId";
				$res = mysql_query($sql) or die("Error in Query: ".mysql_error());
			}


			$sql = "UPDATE tbl_Auctions SET auction_status = $updateListing WHERE auction_ID = $auctionId";
			$res = mysql_query($sql) or die("Error in Query: ".mysql_error());		
		}
